...
- 1e) 每一個比賽項目必須要有一名或多名裁判。
    ...
    - 1e2) 每一位參賽者必須要做好當裁判的準備。當被要求擔任裁判時，只有在能給出合理理由時，參賽者才可以免除責任（例如：不熟悉這種方塊），由WCA代表決定。處罰：取消一個、數個比賽項目或整場比賽的參賽資格。(參見[規則 2k](regulations:regulation:2k))
- 1f) 每個比賽項目必須要有一名或多名的轉亂員。例外：最少步數解。
    - 1f1) 轉亂員要依照轉亂步驟來轉亂方塊。(參見[規則 A2](regulations:regulation:A2))
    - 1f2) 每一位參賽者必須要做好當轉亂員的準備。當被要求擔任轉亂員時，只有在能給出合理理由時，參賽者才可以免除責任（例如：不熟悉轉動代號），由WCA代表決定。處罰：取消一個、數個比賽項目或整場比賽的參賽資格。(參見[規則 2k](regulations:regulation:2k))
...
- 2c) 參賽者報名時要提供所有主辦團隊要求的資料（包括：姓名、代表的地區、出生日、性別、聯絡資訊、欲參加的項目）。
    - 2c1) 如果參賽者未完成有效的報名，將無法取得參賽的資格，取決於主辦團隊。
    - 2c2) 非首次參賽的參賽者，必須要提供和WCA個人檔案相符的個人資訊。如果參賽者希望在兩場比賽之間更改或更新個人資訊（例如：更改地區、姓名或性別），參賽者必須要[聯絡WCA成績組](https://www.worldcubeassociation.org/contact?contactRecipient=wrt)。
- 2d) 參賽者的姓名、代表的地區、性別、和比賽成績視為公開資料。其餘所有個人資料皆被視為保密資料，且必須在取得參賽者同意後才能將資料提供給第三方組織或個人。
- 2e) 參賽者必須代表一個他擁有公民權的地區。WCA代表應該要在該參賽者的第一場比賽以文件（例如：護照）方式確認其公民權。如果參賽者被發現無法合法代表他報名資訊中的地區，參賽者可以被事後判決失去資格和/或禁賽，由WCA董事會決定。
    - 2e1) 有效的地區定義如下[WCA List of Recognized Regions (WCA認可地區清單)](https://www.worldcubeassociation.org/regulations/countries/)。
    - 2e2) 變更地區的參賽者，可以在該年度的第一場比賽當中或之前更改地區。
        2e2a) 例外：如果參賽者喪失了之前所代表的地區的公民權，他們必須在下一場比賽當中或之前更改代表的地區。(參見[規則 2c2](regulations:regulation:2c2))
        2e2b) 例外：如果參賽者在該年度取得公民權，他們可以在任意比賽之間更改代表的地區為新獲得公民權的地區。
...
- 2h) 參賽者在比賽會場內必須要衣著得體。WCA代表可以判定服裝不合宜的參賽者失去比賽資格。
...
- 2s) 有特殊狀況（例如：身心障礙）的參賽者無法遵守一條或多條WCA競賽規則時，可以向WCA代表請求特別協助。有此需求的參賽者應該要在比賽開始前至少兩週聯絡主辦團隊和WCA代表。
    2s1) 對於因特殊情況而無法遵守常規比賽程序的參賽者，可以獲得特殊照顧。這些案件應在申請之前提交給WCA規則委員會。
...
- 3h) 不允許對方塊進行違反基本概念的改造，若修改後的方塊相較於修改前的方塊，不會給參賽者在比賽中提供更多資訊（例如：方塊零件的方向、直接分辨方塊的零件），此方塊便允許使用。
    ...
    - 3h4) 魔錶Clock允許以下的改造：
        - 3h4a) 允許使用自製圖案的紙張（與原本紙張相同形狀與尺寸），由WCA代表決定。該圖案紙張必須明確標註與原本圖案相符的12點鐘位置。
    ...
    - 3h5) 鬆動的按鈕（也就是說不用按壓就能靠重力改變狀態的按鈕）不被視為合理的使用磨損，所以有這種按鈕的魔錶Clock是禁止被使用的。
- 3j) 方塊必須要乾淨，且禁止有任何記號、凸出的部位、損壞、或是任何可以讓各零件更容易區分的相異處。例外：一個logo。(參見[規則 3l](regulations:regulation:3l))
    - 3j1) 方塊上合理的使用磨損是允許的，由WCA代表決定。
        3j1a) 如果磨損是正常使用方塊的結果，並且沒有提供有關方塊狀態的重要附加資訊（例如：有關特定零件的位置），則應該認為磨損是合理的。
...
- 3l) 方塊上的有色部分可以有一個或多個logo。如果有，則logo只能出現在最多一个有色部分上面。例外：盲解項目使用的方塊禁止有logo。
...
- 4g) 轉亂完一顆方塊後，轉亂員必須確認方塊有被正確轉亂。如果方塊的轉亂狀態不正確，方塊必須被修正（例如：先還原方塊，再轉亂一次）。
    - 4g1) 以下例外情況適用：
        ...
        - 4g1b) 5x5x5方塊，轉亂員應該修正轉亂狀態。如果修正錯誤的轉亂狀態會導致嚴重的賽程拖延，則可以不必修正錯誤，由WCA代表決定。
...
- 9u) 如果以下條件全部同時達成，一場比賽將被視為已結束：
    ...
    - 9u2) WCA代表已宣布比賽完成。如果WCA代表沒有宣布比賽完成，則以下任一條件取代此宣告（以先達到者為準）：
...
- A1) 以速解為目標的比賽還原必須要遵守以下流程：
    - A1a) 主辦團隊必須對每一輪做出至少一種時間限制（單次時間限制和/或累積時間限制）。
        ...
        - A1a2) 最少步數解和3x3x3多顆盲解以外的比賽項目，主辦團隊可以採用累積時間限制，累積時間限制必須適用於一輪中所有的還原（例如：3次還原的累積時間限制須低於20分鐘），或是不同比賽項目所有還原的時間總和。在這情況下，某一次還原的時間限制就是設定的單次時間限制或是累積時間限制減掉已用掉的還原時間總和，之中較短的時間。(參見[規則 A1a5](regulations:regulation:A1a5))。若某一輪比賽有累積時間限制，必須只有一種累積時間限制。
...
- A3) 觀察：
    ...
    - A3b) 必要時，裁判將計時器開啟且歸零。如果使用的計時器是Gen5 Pro計時器，裁判必須要確認計時器是設定到 2-Pad 模式。另外，裁判需準備碼錶紀錄觀察時間。參見 [附則 A4d](regulations:regulation:A4d)
        - A3b1) 當裁判認為參賽者準備完畢時，此時裁判會詢問說「準備好了嗎？(Ready?)」。參賽者必須要在被裁判詢問後的1分鐘內準備完畢，否則參賽者失去此次還原機會（DNS），由裁判決定。
        - A3b2) 當參賽者表示準備好後，裁判將方塊上的蓋子移除，裁判同時啟動碼錶計時。
        - A3b3) 當觀察時間到達8秒鐘時，裁判說出「8秒(8 seconds)」。
        - A3b4) 當觀察時間到達12秒鐘時，裁判說出「12秒(12 seconds)」。
        - A3b5) 當選手抬手並啟動計時器，裁判員即停止計時。
    - A3c) 在觀察階段，參賽者可以拿起方塊。
...
- A4) 開始解方塊：
    ...
    - A4d) 如果是使用Stackmat計時器，參賽者應該在綠燈亮起前保持手指接觸計時器感應區。當他們將手從計時器上移開時，計時器就會啟動。參賽者透過啟動計時器開始解方塊。
...
- A7) 成績記錄：
    ...
    - A7b) 裁判在成績單上寫下成績並在成績旁邊簽名以確認成績正確、格式正確，且容易辨認。
        - A7b1) 當有處罰時，裁判寫下計時器上的時間，並在旁邊寫下處罰。格式應該為「T + X = F」，T是計時器顯示的時間（原始時間），X是總加時處罰，F是最後結果成績（例如：17.65 + 4 = 21.65）。
...
- B2) 開始還原：
    ...
    - B2d) 使用Stackmat計時器時，參賽者應該在綠燈亮起前保持手指接觸計時器感應區。當他們將手從計時器上移開時，計時器就會啟動。參賽者透過啟動計時器開始解方塊。
...
- E3) 參賽者可以使用以下的物品。擅自使用其他物品的處罰：判定該次還原未完成（DNF）。
    ...
    - E3b) 3x3x3方塊（最多三顆，自行準備），必須按照[第3章](regulations:article:3)所描述。
    - E3c) 貼紙（自行準備）。
...
- F3) 參賽者在觀察過程中，可以改變任何按鈕的狀態。這些改變不視為轉動。
- F4) 參賽者在觀察過程中，不能改變錶盤的狀態。處罰：判定該次還原未完成（DNF）。
