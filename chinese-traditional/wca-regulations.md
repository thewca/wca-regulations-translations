...
- 3h) 不允許對方塊進行違反基本概念的改造，若修改後的方塊相較於修改前的方塊，不會給參賽者在比賽中提供更多資訊（例如：方塊零件的方向、直接分辨方塊的零件），此方塊便允許使用。
    ...
    - 3h2) 可以由內部辨認外表顏色的方塊（例如：六色方塊）是允許使用的。但不包含以下方塊：
        - 3h2a) 有透明零件的方塊。例外：半透明的貼紙。(參見[規則 3l](regulations:regulation:3l))
...
- 7b) 比賽進行時，觀眾必須要距離正在比賽的舞台至少1.5公尺遠。
- 7c) 比賽場地的照明必須被特別留意，應該使用中性顏色的光，這樣參賽者才能容易辨別方塊上的顏色。
- 7e) 比賽場地必須要禁菸。
- 7f) 比賽區域：
    - 7f1) 定義：
...
- A1) 以速解為目標的比賽還原必須要遵守以下流程：
    ...
    - A1c) 參賽者必須要能夠達成比賽項目基本要求才來參加該項目（例如：知道如何還原該方塊）。參賽者禁止以故意DNF或是故意比出爛成績的心態來參賽。處罰：判定該次還原未完成（DNF）或是取消該比賽項目資格(參見[規則 2j](regulations:regulation:2j))，由WCA代表決定。
- A2) 轉亂：
    ...
    - A2d) 當裁判從轉亂員手上接過方塊後，裁判要快速的看一下方塊的轉亂情形。如果有疑問，裁判應該通知轉亂員做詳細檢查。
        - A2d1) 主辦團隊應該要求轉亂員在成績單上簽名或縮寫表示轉亂員有使用正確的轉亂步驟且轉亂狀態與轉亂步驟的圖案相符。
...
- A3) 觀察：
    ...
    - A3b) 必要時，裁判將計時器開啟且歸零。另外，裁判需準備碼錶紀錄觀察時間。
        - A3b1) 當裁判認為參賽者準備完畢時，此時裁判問說「準備好了嗎？(Ready?)」。參賽者必須要在1分鐘內準備完畢，否則參賽者失去此次還原機會（DNS），由裁判決定。
...
- B5) 結束：
    ...
    - B5c) 在停止計時器前，只要參賽者不碰觸方塊，參賽者可以移除眼罩。在還原結束前，參賽者禁止碰觸方塊。處罰：判定該次還原未完成（DNF）。
