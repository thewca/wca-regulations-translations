...
- 2c+) [附加] 第一次參賽的參賽者必須使用自己的正式名字，或他們在現實生活中傾向被稱呼的名字。姓氏必須是正式名字的姓氏。
- 2c2+) [釐清] 一個WCA資料裡包含了跟參賽者相關的資料，其中含有個人資訊（名字，代表的地區，性別，出生年月日）和正式WCA比賽的所有成績。
- 2d+) [附加] 生日及聯絡資料應該要嚴格保密。
- 2d++) [建議] 如有第三方（例如：媒體）詢問主辦團隊關於參賽者的聯絡資料，應該要先取得參賽者的同意。
- 2e+) [釐清] 如果參賽者擁有多重國籍，他們可以在第一場比賽中選擇代表哪個地區。
...
- 2i+++) [提醒] 藍芽方塊視為電子設備。
- 2i++++) [釐清] 只有選手將電子設備用於特定目的時才被視為正在使用該設備。
- 2i+++++) [舉例] 不被視為使用電子設備的例子：移動相機、翻轉手機、配戴智慧手錶。
- 2i++++++) [舉例] 被視為使用電子產品的例子：按相機的按鈕、查看手機或智能手機的訊息。
- 2i1b+) [釐清] 這包括已關機或已切斷連線的相關裝置。
- 2i1c+) [釐清] 電子式手用暖暖包被視為電子設備，因此不能在觀察或解方塊階段中使用。但非電子式手用暖暖包可以在還原的過程中使用。
- 2i2+) [釐清] 參賽者可以在任何地方持有或佩戴相機，只要該參賽者看不到相機的即時畫面。
...
- 2s++) [舉例] 如果參賽者的狀況（例如：宗教）不允許他們在特定日期或時間參加比賽，WCA 代表可酌情決定允許其在不同時間參加比賽。
- 2s+++) [舉例] 無法用其中一隻手啟動計時器的參賽者可以得到裁判的幫助來啟動和停止計時器，或者聽不見聲音的參賽者可以以不同的方式收到觀察時間提示。
- 2s1+) [舉例] 此類調整的範例包括：盲人參賽者在3x3x3單手解中使用第二隻手感受方塊的狀態，或在3x3x3盲解的記憶階段使用另一個帶有特殊零件的方塊來記憶。
...
- 3j++) [舉例] 有一個適用於[附則 3j](regulations:article:3j)的視覺化指南(https://drive.google.com/file/d/1m6THsA8fXRN7QFM4ApJbm6eVODKGbMLx/view)。
...
- 4g+) [建議] 對於3x3x3多顆盲解，在轉亂所有方塊後，轉亂員必須檢查不存在重複的轉亂。
- 4g1b+) [釐清] 對於5x5x5盲解，錯誤的方塊轉亂狀態必須被修正。
- 4g1b++) [建議] WCA代表在決定是否需要修正5x5x5方塊的錯誤方塊轉亂狀態時應使用更強的判斷力。
...
- 12g2+) [提醒] 之前的規則在觀察中改變按鈕的狀態會被判定該次還原未完成（DNF）。現在已經不是如此了。
...
- A4d1+) [釐清] 如果觀察剛好花費 15 秒（即 15.00 秒），兩秒的處罰（+2秒）必須被使用。
- A4d2+) [釐清] 如果觀察剛好花費 17 秒（即 17.00 秒），該次還原必須判定為未完成（DNF）。
- A5b+) [釐清] 在觀察或還原過程當中，參賽者可以用身體的任何部位碰觸方塊。例外：3x3x3單手解。(參見[規則 C1b](regulations:regulation:C1b))
- A5b++) [釐清] 如果參賽者的方塊發生方塊零件彈出或脫落(參見[規則 5a](regulations:regulation:5a))，參賽者禁止尋求協助尋找或協助撿回零件，且裁判應該要防止他人的幫忙。如果有人替參賽者撿起了彈出的零件，裁判或參賽者可以要求他們把零件放置在比賽桌面上且不能再繼續幫忙。
- A5b+++) [釐清] WCA代表不應該僅僅因為有人替參賽者撿起了彈出或脫落的零件而給予參賽者額外嘗試機會。
...
- F1+) [提醒] 在過去，參賽者必須把魔錶Clock以站立姿勢放在墊子上。現在不再需要此操作。
