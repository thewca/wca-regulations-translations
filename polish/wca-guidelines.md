...
- 9u2b+) [DODATEK] Data kalendarzowa zakończenia zawodów jest określana na podstawie czasu lokalnego dla lokalizacji, w której odbywały się zawody
...
- 11i2++) [PRZYPOMNIENIE] Chociaż indywidualne wyniki osiągnięte na nieprawidłowo wymieszanych łamigłówkach mogą zostać uznane, Delegat WCA musi zapewnić, że nieprawidłowe pomieszania łamigłówek nie staną się powszechne.
...
- A2a3+) [PRZYPOMNIENIE] Zawodnicy nie powinni mieć pozwolenia na oglądanie sekwencji scramble dla swojej grupy, nawet po ukończeniu wszystkich swoich prób, chyba że Delegat WCA wyrazi na to zgodę. (zob. [Wytyczna 4b2+](guidelines:guideline:4b2+))
...
- A2c2+) [PRZYPOMNIENIE] Jeśli zawodnik przypadkowo obejrzy pomieszanie, którego jeszcze nie otrzymał, nie może otrzymać tego pomieszania jako własną próbę, a zamiast tego musi zostać przyznana dodatkowa próba.
...
- A2d1++) [DODATEK] Próba nie może zostać zdyskwalifikowana wyłącznie z powodu braku podpisu pod pomieszaniem. Jeśli po rozpoczęciu próby brakuje podpisu pod pomieszaniem, sędzia musi zezwolić na kontynuowanie próby (jeśli nadal trwa), a Delegat WCA powinien zezwolić na utrzymanie wyniku.
...
- A3c1+) [WYJAŚNIENIE] W przypadku konkurencji Clock, zawodnik może zmieniać stan pinów w trakcie inspekcji (zob. [Punkt 12i1](regulations:regulation:12i1))
...
- A6b1+) [WYTŁUMACZENIE] Sędziowska wartość 0,06 sekundy została wybrana w celu usystematyzowania nieprawidłowości timera Stackmats.
...
- A6e++) [WYJAŚNIENIE] Jeśli [Punkt A6c](regulations:regulation:A6c) jest użyta do penalizowania próby, kara za [Punkt A6e](regulations:regulation:A6e) powinna być nałożona tylko wtedy, gdy zawodnik wypuści łamigłówkę z rąk i dotknie ją ponownie.
...
- A7c6+) [PRZYKŁAD] Przykłady podpisów identyfikujących zawodnika: Imię i nazwisko, inicjały, WCA ID, ID zawodnika, rozsądny pseudonim.
- A7c6++) [WYJAŚNIENIE] ID zawodnika to identyfikator (np. numer) przypisany zawodnikowi na czas zawodów. Niektóre zawody używają go do śledzenia kart wyników i wprowadzania wyników.
- A7c6+++) [REKOMENDACJA] Zespół organizacyjny może poprosić zawodników o złożenie podpisu przy użyciu jednej konkretnej metody identyfikacji (np. ID zawodnika), ale zawodnicy nie powinni być dyskwalifikowani za użycie innej metody podczas składania podpisu.
- A7c6++++) [DODATEK] Zawodnicy powinni używać tego samego podpisu w trakcie mieszania lub sędziowania w trakcie zawodów.
...
- B1+) [PRZYPOMNIENIE] Zawodnik musi użyć łamigłówki bez tekstur, oznaczeń lub innych cech odróżniających podobne elementy (zob. [Punkt 3k](regulations:regulation:3k)). Należy zwrócić na to szczególną uwagę w przypadku układania bez patrzenia. Wyjątek: Dla konkurencji 3x3x3 Bez Patrzenia, łamigłówki z logiem są dozwolone na podstawie [Punktu 3l](regulations:regulation:3l).
...
- E2c4+) [PRZYKŁAD] Przykłady interpretacji symboli lub kombinacji symboli, które nie są zdefiniowane w [Punkcie12a](regulations:regulation:12a): [x] należy interpretować jako x. (R * U', * = F2) należy interpretować jako R U' F2. (R U)' należy interpretować jako R U'. R++ należy interpretować jako R. R3' jest niedozwolone.
- E2c4++) [DODATEK] Ruchy z apostrofem i 2 powinny być interpretowane tak, jakby nie było apostrofu (np. R2' powinno być interpretowane jako R2).
- E2c6+) [PRZYKŁAD] Y należy interpretować jako y. UW, uw i uW należy interpretować jako Uw. r należy interpretować jako R, a nie Rw.
...
- E3b+) [WYJAŚNIENIE] Łamigłówki nie muszą być ułożone w trakcie rozpoczynania próby.
