...
- 1h1+) [WYJAŚNIENIE] Mieszacze / sędziowie mogą mieszać / sędziować w swojej grupie startowej jedynie jeżeli jest to niezbędne do prowadzenia zawodów.
- 1h1++) [WYJAŚNIENIE] W konkurencji Najmniejszą Liczbą Ruchów, sędziowie mogą sędziować zawodników startujących razem z nimi zanim zakończą swoje próby (kiedy jeszcze startują).
...
- 2c2+) [WYJAŚNIENIE] Profil WCA jest zbiorem danych związanych z zawodnikiem, zawiera on dane personalne (imię, region który reprezentują, płeć, oraz datę urodzenia) oraz wszystkie ich wyniki z oficjalnych zawodów WCA.
- 2d+) [DODATEK] Data urodzenia oraz informacje kontaktowe powinny być szczególnie chronione.
- 2d++) [REKOMENDACJA] Jeżeli osoba trzecia (np. dziennikarz) poprosi zespół organizacyjny o informacje kontaktowe do któregokolwiek zawodnika, zawodnik najpierw powinien być poproszony o wyrażenie zgody.
...
- 2e++) [DODATEK] Delegat WCA powinien zweryfikować nazwisko i datę urodzenia za pomocą dokumentów (np. paszportu) każdego nowego zawodnika na jego pierwszych zawodach.
...
- 2i++) [DODATEK] Zespół organizujący może zapewnić drugi stoper do sprawdzania czasu od rozpoczęcia próby (wystartowany jednocześnie z głównym stoperem). W tej sytuacji zawodnik nie może dotykać stopera głównego.
...
- 2i2+) [WYJAŚNIENIE] Zawodnik może trzymać lub nosić kamerę w dowolnym miejscu, o ile podgląd z kamery nie jest widoczny dla zawodnika.
...
- 3l++) [WYJAŚNIENIE] W przeszłości układanki z logo były dozwolone w konkurencjach bez patrzenia. Takie łamigłówki nie są już dozwolone.
...
- 5b+) [DODATEK] Zawodnikowi nie wolno celowo powodować wad układanki, chyba że poprawia wcześniejszą wadę układanki (zob. [Punkt 5b3b](regulations:regulation:5b3b) i [Punkt 5b3c](regulations:regulation:5b3c)). PRZYKŁADs of intentionally causing puzzle defects: removing an attached center cap, rotating a corner piece, peeling a sticker.
...
- 9f5+) [WYJAŚNIENIE] Wynik ułożenia to DNS wtedy, gdy zawodnik był zdolny do rozpoczęcia ułożenia, ale nie zrobił tego (zob. [Punkt A3b2](regulations:regulation:A3b2)). Jeżeli zawodnik nie zakwalifikował się do ułożenia (np. w Rundzie Kombinowanej), nie ma żadnego wyniku.
...
- 9u+) [DODATEK] W przypadku zawodów z wieloma fizycznymi lokalizacjami (np. "Zawody 3x3x3 Fewest Moves symultaniczne"), wszystkie wymagania muszą zostać spełnione w każdej lokalizacji, aby zawody zostały uznane za zakończone.
