...
- 9b) WCA 公式競技種目は以下の通りである。
    - 9b1) 3x3x3 キューブ、2x2x2 キューブ、4x4x4 キューブ、5x5x5 キューブ、3x3x3 片手、3x3x3 足、クロック、メガミンクス、ピラミンクス、スキューブ、スクエア1。
    ...
    - 9b2) 6x6x6 キューブ、7x7x7 キューブ。
        ...
        - 9b2b) これらの種目の複合ラウンド形式：カットオフ段階が「Best of 1」の「Mean of 3」。
    ...
    - 9b4) 3x3x3 最少手数。
        ...
        - 9b4b) この種目の複合ラウンド形式：カットオフ段階が「Best of 1」の「Best of 2」、またはカットオフ段階が「Best of X」（X は 1 または 2）の「Mean of 3」。
    - 9b5) 3x3x3 複数目隠し。
        - 9b5a) この種目のラウンド形式：「Best of X」（X は 1、2、または 3）。
        - 9b5b) この種目の複合ラウンド形式：カットオフ段階が「Best of X」（X は 1 または 2）の「Best of Y」（Y は 2 または 3 で Y > X）。
...
- 9g) 複合ラウンドは「Best of X」のカットオフ段階とカットオフ基準を持つラウンドである（例：カットオフ基準が 2 分の「Best of 2」。）競技者がカットオフフェーズの試技のうち少なくとも 1 つでカットオフ基準を満たした場合、残りの試技を行うことができる。カットオフ段階の試技はラウンド全体の試技に含まれる。
...
- 9p) ラウンドが複数回行われる場合、以下に従う。
    ...
    - 9p3) 出場資格のある競技者が棄権する場合、先行するラウンドで敗退した中で最上位の競技者が代わりに出場してもよい。
...
- 12a) NxNxN キューブの記号を以下のように定める。
    ...
    - 12a4) 持ち替え（パズルそのもの）
        - 12a4a) 時計回りに 90 度：[f] または z、[b] または z'、[r] または x、[l] または x'、[u] または y、[d] または y'。
        - 12a4b) 反時計回りに 90 度：[f'] または z'、[b'] または z、[r'] または x'、[l'] または x、[u'] または y'、[d'] または y。
        - 12a4c) 180 度：[f2] または z2、[b2] または z2、[r2] または x2、[l2] または x2、[u2] または y2、[d2] または y2。
...
- A1) スピード競技の試技は、以下の進行手続きを守らなければならない。
    - A1a) 大会運営チームは、それぞれのラウンドに、統一された制限タイム（試技ごとの制限タイムまたは累積方式の制限タイム）を設けなければならない。
        ...
        - A1a2) 大会運営チームは 3x3x3 最少手数および 3x3x3 複数目隠しを除く種目にて制限タイムを累積方式とすることができる。累積方式の制限タイムは 1 ラウンド内の全ての試技に適用するか（例：3 ソルブで 20 分という制限タイム）か、または異なる種目のラウンドの組み合わせすべての試技に適用しなければならない。その場合、ある試技の制限タイムは累積方式の制限タイムからその試技より前の全ての試技の合計時間を引いたものとなる。（参照：[規則 A1a5](regulations:regulation:A1a5)）。ラウンドが適用可能な累積方式の制限タイムを持つ場合、最大で 1 つでなければならない。
    ...
    - A1c) 競技者は出場種目に参加するための条件（例：そのパズルの解き方を知っている）を満たしていなければならない。DNF や意図的に悪い結果が予期される場合は競技してはならない。罰則：WCA 代理人の裁量によって、試技の失格（DNF）または種目の失格。
- A2) スクランブル：
    ...
    - A2d) ジャッジはパズルをスクランブラから受け取ったときに、簡単にパズルを確認する。ジャッジがスクランブラに懸念を提示したら、スクランブラは詳細に確認する。
        - A2d1) 大会運営チームはスクランブラに対し、正しい番号のスクランブル手順をスクランブルしたかや、パズルがスクランブル状態の図と一致しているか確認した旨を、記録用紙に署名する（イニシャル可）ことを要請することが望ましい。
...
- A3) インスペクション：
    ...
    - A3b) ジャッジはタイマーの電源を入れて、必要に応じてリセットして準備することによりタイマーの準備をする。また、場合によってはインスペクションのためにストップウォッチの準備をする。
        - A3b1) 競技者の用意ができているだろうと判断した際、ジャッジは競技者に「READY?（準備はよろしいですか？）」と尋ねる。競技者は呼びかけから 1 分以内に試技開始の準備をしなければならない。1 分以内に準備をしない場合、ジャッジの裁量により試技の権利を失う（DNS）。
...
- E2)  3x3x3 最少手数の進行手続き：
    ...
    - E2b) 全競技者は解法を探して記述する時間として、60 分間の制限タイムが与えられる。
