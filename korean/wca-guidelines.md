...
- 3k2b+) [명료화] 3x3x3 멀티 블라인드 시도 도중 허용되지 않은 퍼즐이 발견될 경우, 퍼즐은 절대로 교체되거나 시도에서 제거되어서는 안 되며, 전체 시도가 실격 처리되지 않는 한 반드시 맞추어지지 않은 것으로 집계되어야 한다.
...
- 3l++) [명료화] 3x3x3 블라인드 종목에서는 로고가 허용되지만, 3x3x3 멀티 블라인드 종목을 포함한 다른 블라인드 종목에서는 허용되지 않는다.
...
- 4g1b+) [명료화] 5x5x5 블라인드 종목에서, 잘못된 퍼즐 상태는 반드시 수정되어야 한다.
...
- 9f2+) [추가] 3x3x3 멀티 블라인드 종목에서, 심판은 맞추어진 퍼즐의 개수, 시도한 퍼즐의 개수, 최종 시간을 기록해야 한다. 예시로, 선수가 40분 30초 동안 5개의 큐브를 시도하여 3개의 큐브를 맞춘 경우, 심판은 "3/5 40:30"라고 작성한다.
...
- B1+) [상기] 선수는 반드시 비슷한 조각끼리 구별([규정 3k](regulations:regulation:3k) 참고)할 수 있게 하는 질감, 표시, 혹은 다른 특징이 없는 퍼즐을 사용해야 한다. 이는 눈 가리고 맞추기에서 각별한 주의가 필요하다. 예외: 3x3x3 블라인드 종목의 경우, [규정 3l](regulations:regulation:3l)에 의해 허용된 로고가 퍼즐에 있을 수 있다.
...
- B4d+) [명료화] 3x3x3 블라인드 종목의 경우, 눈을 가리고 맞추는 단계 동안 선수가 촉감으로 허용된 로고([규정 3l](regulations:regulation:3l) 참고)의 위치를 판단하는 것이 허용된다.
...
- H1b+) [추가] WCA 파견위원의 재량에 따라, 선수는 이미 진행 중인 멀티 블라인드 시도에 참여할 수 있다. 이 경우, 해당 선수의 시간제한은 남은 시간만큼이다(즉, 다른 선수들이 시간제한에 도달할 때 반드시 함께 마쳐야 한다). 예외: 다른 선수가 이미 시도를 완료했거나 참여하려는 선수가 스크램블 이미 알게 되었을 가능성이 있는 경우, 해당 선수는 멀티 블라인드 시도에 절대로 참여할 수 없다.
- H1b1+) [상기] 멀티 블라인드 종목에 대한 예외([규정 A1a4](regulations:regulation:A1a4)와 [규정 A1a5](regulations:regulation:A1a5) 참고)에 따라, 제한 시간에 도달하여도 시도는 실격 처리되지 않는다.
