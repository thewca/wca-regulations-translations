...
- 3h) 퍼즐의 기본 개념을 향상하는 개조는 허용되지 않는다. 퍼즐의 개조된 버전은 동일한 퍼즐의 개조되지 않은 버전과 비교했을 때 선수가 추가적인 정보(예: 조각의 오리엔테이션 또는 유형)를 확인할 수 없는 경우에만 허용된다.
    ...
    - 3h4) 클락 종목에서 다음 개조는 허용된다:
        - 3h4a) WCA 파견위원의 재량에 따라 커스텀 "삽입물"(원래 삽입된 것과 동일한 모양과 크기)은 허용된다. 삽입물은 원래의 삽입물에 일치하도록 12시 방향 표시가 반드시 명확히 있어야 한다.
...
- 9f) 라운드의 결과는 다음과 같이 측정된다:
    ...
    - 9f2) 3x3x3 멀티 블라인드 종목의 모든 결과를 포함한 10분을 초과하는 측정된 모든 결과는 1초 단위로 절사된다. 측정된 10분 이상의 모든 절단 평균과 평균은 가장 가까운 1초 단위로 반올림되어 기록된다(e.g. X.49는 X, X.50은 X+1).
