...
- 2u) Участник должен присутствовать и быть готовым, когда его вызывают для участия. Наказание: дисквалификация с дисциплины.
    - 2u1) Исключение: если участник не присутствует перед индивидуально запланированной попыткой (например, попытка Сборки 3x3x3 на количество ходов, 3x3x3 мульти-блайнд), то по решению делегата WCA участнику можно поставить DNS, как если бы он отказался от этой попытки.
...
- 3k) Перед использованием на соревновании делегату следует одобрить головоломки участников.
    ...
    - 3k2) Наказание для попыток, в которых использовалась недопустимая головоломка: дисквалификация попытки (DNF). Исключения:
        ...
        - 3k2b) 3x3x3 мульти-блайнд: если были обнаружены недопустимые головоломки, то по усмотрению делегата WCA они могут быть засчитаны как несобранные (вместо того, чтобы дисквалифицировать попытку целиком).
...
- A1) Попытки в скоростной сборке должны проходить по следующей процедуре:
    - A1a) Команда организаторов должна установить минимум один из следующих видов лимитов по времени для сборок: лимит на каждую попытку и/или суммарный лимит. Лимит должен быть установлен для каждого раунда.
        ...
        - A1a4) Участник должен заканчивать сборки до того, как истечёт лимит по времени. Если время на таймере участника превысило лимит по времени, судья немедленно останавливает попытку и ставит DNF. Исключение: 3x3x3 мульти-блайнд (см. [Положение H1b1](regulations:regulation:H1b1)).
...
- A6) Окончание сборки:
    ...
    - A6c) Участник должен полностью отпустить головоломку перед остановкой сборки. Штраф: дисквалификация попытки (DNF). Исключение: если участник не делал ходов и не выравнивал грани головоломки после остановки таймера и до того, как он отпустил головоломку, по решению судьи штраф заменяется на +2 секунды.
...
- E2) Процедура Сборки 3xx3 на количество ходов:
...
## <article-H><multiple-blindfolded><multipleblindfoldedsolving> Статья H: 3x3x3 мульти-блайнд (сборка вслепую нескольких кубов)
