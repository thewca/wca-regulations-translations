...
<version>Версия: 1 августа 2020 г.
...
- 7h) В зоне для соревнования должна быть по крайней мере одна зона для участников (замещено [Положением Y6a](regulations:regulation:Y6a)).
...
- A7) Запись результатов:
    ...
    - A7c) Участник должен проверить записанный результат и подписать его (или поставить инициалы) на карточке, принимая, что результат верный, полный, правильно отформатирован и хорошо читаемый. После этого попытка заканчивается (частично замещено [Положением Y7e1](regulations:regulation:Y7e1)).
...
- F2) Судья ставит перемешанную головоломку на мат в вертикальном положении.
    - F2a) Команда организаторов может заставить использовать подставки для головоломки, чтобы она не падала до начала попытки. Если подставки применяются, об этом должно быть объявлено до начала раунда.
...
- Y5) Судьи могут принести свой собственный секундомер или телефон для измерения времени преинспекции, которые следует проверить Делегату WCA.
- Y6) Окружающая обстановка:
    - Y6a) Если Делегат WCA считает это необходимым, в месте проведения соревнования не должно быть зоны для участников.
...
- Y7) Ниже описаны дополнительные процедуры, замещающие соответствующие положения в [Статье A](regulations:article:A), [Статье B](regulations:article:B) и [Статье H](regulations:article:H). Эти процедуры не применимы к [Статье E](regulations:article:E), Сборке на количество ходов.
    ...
    - Y7c) Перемешка головоломок:
        - Y7c1) Когда участника вызывают для участия в раунде, после того как он продезинфицировал свою головоломку, участник сдает её скрамблеру, в собранном состоянии. Участник затем ждёт своих попыток за своим местом.
    - Y7d) Преинспекция:
        - Y7d1) Судья просит участника приготовить таймер, включив его и сбросив при необходимости. Участник затем включает и сбрасывает таймер, если необходимо. Судье не следует касаться таймера.
    - Y7e) Запись результатов:
        - Y7e1) Участник должен проверить записанный результат и подписать его (или поставить инициалы) на карточке собственной ручкой, принимая, что результат верный, полный, правильно отформатирован и хорошо читаемый.
