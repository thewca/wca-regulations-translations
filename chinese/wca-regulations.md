...
- 4g) 打乱完成后，打乱员必须检查魔方是否被正确地打乱了。如果打乱状态是错的，该打乱员必须纠正它（例如将魔方还原后重新打乱）。
    ...
    - 4g3) 在确认魔方已正确打乱后，打乱员可以将魔方交给另一名打乱员进行验证。在这种情况下，两名打乱员都应在成绩单上签名。
    - 4g3+) [建议] 在国家锦标赛、大洲锦标赛或者世界锦标赛的决赛中，所有打乱后的魔方应该由另一名打乱员进行检查。
...
- 9f) 一轮比赛的比赛结果按以下方式衡量和记录：
    ...
    - 9f6) 在“X次取最快”的比赛形式中，每个选手有X次还原机会，这些还原中最好的成绩作为选手在该轮排名的依据。例外：在双重轮中（见[规则9v4](regulations:regulation:9v4)），选手的名次由其在任意一轮中获得的更好的成绩决定。
    - 9f7) 在“X次取最快”的比赛形式中，DNF或DNS视为最坏成绩。
    - 9f8) 在“5次取去尾平均”的比赛形式中，每个选手有5次还原机会，在这5次还原中去掉最好和最坏的成绩，剩下3次成绩的算术平均值作为选手在该轮排名的依据。例外：在双重轮中（见[规则9v4](regulations:regulation:9v4)），选手的名次由其在任意一轮中获得的更好的平均成绩决定。
    - 9f9) 在“5次取去尾平均”的比赛形式中，DNF或DNS视为最坏成绩。如果一个选手有不止一个DNF和/或DNS，则他该轮的平均成绩为DNF。
    - 9f10) 在“3次取平均”的比赛形式中，每个选手有3次还原机会，这3次成绩的算术平均值作为选手在该轮排名的依据。例外：在双重轮中（见[规则9v4](regulations:regulation:9v4)），选手的名次由其在任意一轮中获得的更好的平均成绩决定。
...
- 9l) 所有项目当轮的比赛必须在该项目下一轮比赛可以开始前结束。以下情形例外：
    ...
    - 9l2) 对于双重轮，第二轮可以在第一轮结束前开始。这是因为晋级至第二轮不依赖于第一轮的成绩（见[规则 9v5](regulations:regulation:9v5)）。
- 9m) 所有项目至多只能有四轮。
...
- 9o) 按以下方式计算项目轮数：
    ...
    - 9o3) “一对一”轮次记为一轮。
- 9p) 如果某个项目有多轮，那么：
    - 9p1) 同一项目相邻两轮间至少有25%的选手被淘汰。例外：双重轮（见[规则9v5](regulations:regulation:9v5)）。
...
- 9t++) [附加] 如果在比赛过程中出现了这些改变成绩的情况，这条规则也必须被使用。在这种情况下，[9t1](regulations:regulation:9t1)、[9t2](regulations:regulation:9t2)以及[9t3](regulations:regulation:9t3)也应该适用, 由WCA代表决定。
    ...
    - 9t5) 例外：如果一名选手在比赛结束后由于[规则11i1](regulations:regulation:11i1)而将成绩更改为DNS，从而不再晋级，则后续轮次的成绩不会被移除。
...
- 9v) 一个项目的连续两轮可以被设计为双重轮。
    - 9v1) 双重轮只能包含一个项目的前两轮。
    - 9v2) 双重轮不能包含国家锦标赛、大洲锦标赛或者世界锦标赛的决赛。
    - 9v3) 双重轮必须具有相同的比赛形式、及格线以及时限。
    - 9v4) 选手在双重轮中的排名由他们在这两轮中较好的成绩排序得出。
        - 9v4a) 选手在这两轮中的成绩都会被记录，并且都可以参与WCA世界排名（见[规则9i](regulations:regulation:9i)）以及计算地区纪录（见[规则9i1](regulations:regulation:9i1)）。
    - 9v5) 在双重轮的这两轮之间不会淘汰任何选手。
...
- 11i) 如果在选手完成复原之后发现其魔方没有被正确打乱，以下规则适用：
    ...
    - 11i1+) [建议] 预期符合这些标准的选手，在送出其打乱魔方之前应由第二名打乱员检查其打乱情况（见[规则4g3](regulations:regulation:4g3)）。
    - 11i1++) [建议] 对于符合这些条件的成绩，WCA代表应该在比赛结束前尝试进一步验证，例如使用可用的视频证据，确认魔方被正确打乱。
    - 11i1+++) [附加] 如果有符合这些条件的成绩的选手提出要求，WCA代表必须在比赛结束前提供相关的打乱公式。
        ...
        - 11i1e) [11i1a](regulations:regulation:11i1a)和[11i1b](regulations:regulation:11i1b)的例外：对于五阶魔方、六阶魔方、七阶魔方、五魔方以及三阶多盲项目，适用[规则11i2](regulations:regulation:11i2)。
        - 11i1f) [11i1a](regulations:regulation:11i1a)和[11i1b](regulations:regulation:11i1b)的例外：对于在国家锦标赛、大洲锦标赛或者世界锦标赛的决赛中取得的成绩，如果两名打乱员检查认为魔方已被正确打乱并且已在成绩单上签名（见[规则4g3](regulations:regulation:4g3)），则[规则11i2](regulations:regulation:11i2)适用。
...
- A1) 速拧还原必须遵循以下过程。
    ...
    - A1a+) [澄清] 如果两种时限都使用了，单次还原的时限不得比累计还原时限更长。
        ...
        - A1a2) 除三阶最少步和三阶多盲外的项目，主办团队也可以采用累计时限。累计还原时限必须应用在一轮中的所有尝试（例如3次还原的累计时间不超过20分钟），或者应用于在一个项目的双重轮中的所有尝试（见[规则9v](regulations:regulation:9v)），亦或应用于不同项目的轮次组合中的所有尝试。在这种情况下，某一次特定的还原的时限是它自己的时限或者总的还原时限减去到目前为止已经消耗的时限（见[规则A1a5](regulations:regulation:A1a5)），以更短的为准。如果一个轮次有适用的累计还原时限，它最多只能有一个。
