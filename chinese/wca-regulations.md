...
- 2c) 选手在报名时需要提交所有主办团队要求的信息（包括：姓名、代表的地区、生日、性别、联系方式、项目）。
    ...
    - 2c2) 参加过比赛的选手提供的个人信息必须与他们的WCA资料一致。如果选手希望在比赛之间更改或者更新个人信息（如更改代表的地区、姓名、性别等），则必须直接或通过WCA代表[联系WCA成绩组](https://www.worldcubeassociation.org/contact?contactRecipient=wrt)。
...
- 3l) 魔方的有色部分可以有一个或多个logo。如果有，则logo只能出现在最多一个有色部分上面。例外：在盲拧项目中，魔方不允许有logo。
