...
- 3h) 禁止对魔方进行超常的修改。对魔方的合法修改必须满足：这一修改相比于未修改的同类魔方无法给选手提供任何附加信息（例如区分两个块）。
    ...
    - 3h5) 魔表上松动的立柱（例如会因为重力而不是人为按压就改变状态的立柱）不被认为是合理的磨损，这样的魔表不被允许。
...
- 4g) 打乱完成后，打乱员必须检查魔方是否被正确地打乱了。如果打乱状态是错的，该打乱员必须纠正它（例如将魔方还原后重新打乱）。
    ...
    - 4g2) 例外：对于魔表，如果按钮的状态在运送或者去掉遮挡的时候被影响了，这些错误的状态可以不用纠正。
...
- 9f) 一轮比赛的比赛结果按以下方式衡量和记录：
    ...
    - 9f4) 如果还原被取消资格或没有还原，则该结果被记录为DNF（未还原）。
        - 9f4a) 当选手明显违反了规则，这次尝试确定要被取消资格的时候，裁判应当立即停止这次还原尝试。如果裁判不确定或者可能会对比赛造成其他干扰时（例如：额外增加一次复原会拖延比赛），裁判应当咨询WCA代表。
...
- 9u) 当以下所有条件都满足时，比赛结束：
    ...
    - 9u2) WCA代表宣布比赛结束。如果WCA代表没有宣布比赛结束，则需要下列条件中至少一个被满足：
...
- B2) 开始还原：
    ...
    - B2d) 当使用Stackmat计时器的时候，选手通过以下过程开始还原：确认计时器灯变绿，然后将他的手从计时器上移开，即启动计时器（同时开始了此次还原）。
