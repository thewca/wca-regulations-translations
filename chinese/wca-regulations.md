...
- 2i++) [附加] 主办方可以向选手提供一个非正式的秒表用于察看已消耗的时间（和正式的秒表一起启动），这种情况下选手不能触碰正式的秒表。例外： 见[规则2i+](regulations:regulation:2i+)的例外.
- 2i+++) [提醒] 蓝牙魔方被认为是电子设备。
...
- 9f) 一轮比赛的比赛结果按以下方式衡量和记录：
    ...
    - 9f2) 所有超过10分钟的计时结果以及三阶多盲的时间，向下取整至秒。所有超过10分钟的平均、去尾平均结果四舍五入取整至秒（例如：X.49计为X，而X.50计为X+1）。
...
- A1) 速拧还原必须遵循以下过程。
    ...
    - A1a+) [澄清] 如果两种时限都使用了，单次还原的时限不得比累计还原时限更长。
        ...
        - A1a2) 除三阶最少步和三阶多盲外的项目，主办团队也可以采用累计时限。累计还原时限必须应用在一轮中的所有尝试（例如3次还原的累计时间不超过20分钟），或应用于不同项目的轮次组合种的所有尝试。在这种情况下，某一次特定的还原的时限是它自己的时限或者总的还原时限减去到目前为止已经消耗的时限（见[规则A1a5](regulations:regulation:A1a5)），以更短的为准。如果一个轮次有适用的累计还原时限，它最多只能有一个。
    ...
    - A1b) 如果一次尝试的时限超过10分钟，需要使用秒表用于计时。
        ...
        - A1b2) 如果Stackmat计时器的时间可用，则依此记录时间。否则（例如：Stackmat计时器出现了故障、超过了10分钟或根本没有被使用）依秒表记录时间。
...
- A2) 打乱：
    - A2a) 当被叫到参加某一轮，选手将一个处于还原状态的魔方交给打乱员。
        ...
        - A2a2) 如果没有使用选手等候区，选手必须一直待在指定的赛台，直到完成该轮的所有尝试。例外：选手可以在获得WCA代表许可的情况下离开指定的赛台。惩罚：取消其未完成的剩余尝试的资格（DNF），由WCA代表决定。
    ...
    - A2c) 打乱员开始打乱魔方后，直到观察阶段开始前选手禁止看到魔方。
        ...
        - A2c2) 在比赛过程中，选手不得主动观察其他正在进行正式比赛的选手的复原过程。如果选手观察了其他选手复原自己尚未收到的打乱，则该次尝试将被记录为DNF。
    ...
    - A2d) 从打乱员处接过魔方后，裁判可以粗略的看一下魔方以确保它被充分打乱。裁判可以向打乱员提出任何疑问，后者将进行更细致的检查。
        - A2d1) 打乱员或被授权的裁判必须检查打乱的魔方，并且在成绩单上签名，以确保执行了正确的打乱，且打乱状态与打乱图案匹配（见[规则4g](regulations:regulation:4g)）。例外：对于六阶、七阶魔方及五魔方，打乱员可以通过签名确认魔方已经被充分打乱（见[规则4g1](regulations:regulation:4g1)）。打乱员的签名应当在整场比赛中保持一致，并且可以通过签名识别出具体的打乱人员。
...
- A3) 观察：
    ...
    - A3d) 在完成观察后，选手将魔方以任意朝向放在垫子上。魔方必须完全放置在垫子上，且不得完全或部分地压在Stackmat计时器上。惩罚：加罚时间（+2秒）。
- A4) 开始还原：
...
- E2) 三阶最少步过程：
    - E2a) 裁判将打乱公式以及纸张发给所有参赛选手。然后裁判启动秒表并说“开始”。
        - E2a1) 尝试开始前，选手不能在尝试相关的任何纸张上书写。例外：选手可以写下识别这次尝试的信息（见[规则E2c1](regulations:regulation:E2c1)）。如果他在包含打乱的纸上写这个，他在尝试前只能写在不包含打乱的那一面。对书写任何非识别这次尝试信息的惩罚：取消此次还原资格（DNF）。
