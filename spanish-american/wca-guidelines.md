...
- 2j2++) [EJEMPLO] Si el delegado de la WCA descalifica a un competidor durante su tercer intento en una ronda, solo son descalificados el tercer intento y todos los siguientes del evento, incluso si las circunstancias que originaron la descalificación ocurrieron antes del tercer intento.
...
- 3l++) [ACLARACIÓN] En el pasado los puzzles con logos estaban permitidos para los eventos a ciegas. Esto ya no se permite más.
...
- 4b1+) [RECORDATORIO] El delegado de la WCA no debe regenerar nunca las mezclas para reemplazar otros con el propósito de filtrar. Por ejemplo, no está permitido mirar las mezclas de una competición y generar de nuevo el lote entero con el propósito de generar mezclas "más justas".
...
- 4f+) [RECOMENDACIÓN] El delegado de la WCA debería generar mezclas suficientes para la totalidad de la competición antes de tiempo, incluyendo mezclas de sobra para intentos extra.
- 4f++) [RECORDATORIO] Si el delegado de la WCA genera mezclas oficiales durante la competición, las mezclas deben guardarse.
- 4g2+) [ACLARACIÓN] De todas formas, el mezclador debe verificar que el Clock esté mezclado correctamente.
- 4g2++) [ACLARACIÓN] Si el Clock cae sobre la mesa al ser destapado, debe otorgarse un intento extra.
- 4g2+++) [RECOMENDACIÓN] El mezclador debería dejar la hoja de resultados fuera de la cobertura del puzzle.
...
- 9u+) [ADICIÓN] Las competiciones con múltiples localidades físicas (p.ej. "3x3x3 en Menos Movimientos, múltiples ubicaciones"), se consideran terminadas cuando todos los requerimientos se cumplen en todas las localidades.
- 9u2+) [RECOMENDACIÓN] El delegado de la WCA debería anunciar el fin de la competición (p.ej. antes de la ceremonia de premiación).
- 9u2b+) [ADICIÓN] La fecha del fin de la competición está determinado por el tiempo local del lugar donde se desarrolla la competición.
...
- 11e+++++) [ADICIÓN] Si el delegado de la WCA no tiene tiempo para investigar la situación durante la competición, debería otorgar un intento extra provisorio.
...
- A7g++++) [ACLARACIÓN] Se puede considerar que un competidor carece de experiencia si es nuevo, si es la primera vez que compite en un evento en específico o si no ha competido en un evento en específico por un período de tiempo considerable. La falta de experiencia debe estar relacionada al incidente en cuestión.


## <article-B><blindfolded><blindfoldedsolving> Artículo B: Resolución a ciegas

- B1+) [RECORDATORIO] El competidor debe usar un puzzle sin logos (ver [Regla 3l](regulations:regulation:3l)), texturas, marcas u otras características que permitan distinguir piezas similares (ver [Regla 3k](regulations:regulation:3k)). A esto se le debería dar especial atención en la Resolución a Ciegas.
- B1b+) [RECOMENDACIÓN] Los antifaces deberían ser verificados por el delegado de la WCA antes de su uso en la competición.
...
- Bf2+) [ACLARACIÓN] Dado que el competidor inicia el intento al destapar el puzzle, las penalizaciones por iniciar el timer y por tocar la cobertura del puzzle al iniciar el intento no son aplicables (ver [Regla B2b](regulations:regulation:B2b) y [Regla B2c](regulations:regulation:B2c)).
